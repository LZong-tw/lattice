...
- Serena HTTP sidecar launches now hide Windows subprocess windows and pass
  `--enable-web-dashboard false`, `--open-web-dashboard false`, and
  `--enable-gui-log-window false` to avoid surprise dashboards or GUI log
  windows.
- Docs now call out `LATTICE_DISABLE=serena` for projects that already use a
  project-wide Serena HTTP singleton, so Lattice does not launch a redundant
  9122/9123 sidecar.
